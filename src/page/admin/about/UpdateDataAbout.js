...
function UpdateDataAbout() {
  ...
  const sendDataUpdateAbout = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`http://localhost:5000/about/${id}`, {
        nama_lengkap,
        tanggal_lahir,
        alamat,
        email,
  ...
  const getContactById = async () => {
    const aboutByOne = await axios.get(`http://localhost:5000/about/${id}`);
    setNama_lengkap(aboutByOne.data.nama_lengkap);
    setTanggal_lahir(aboutByOne.data.tanggal_lahir);
    setAlamat(aboutByOne.data.alamat);
    setEmail(aboutByOne.data.email);
    setNo_hp_wa(aboutByOne.data.No_hp_wa);
    setSekolah_dasar(aboutByOne.data.sekolah_dasar);
    setSekolah_menengah_pertama(aboutByOne.data.sekolah_menengah_pertama);
    setSekolah_menengah_kejuruan(aboutByOne.data.sekolah_menengah_kejuruan);
    setStrata_satu(aboutByOne.data.strata_satu);
  };
  return (
            ...
            <div className="col-sm-12 col-md-12 col-lg-6 col-xl-6 col-xxl-6 formupdate " style={{ fontSize: "10px", paddingLeft: "40px", paddingTop: "20px" }}>
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", paddingLeft: "20px" }}>
                <div className="col-sm-6 col-md-6 col-lg-10 col-xl-10 col-xxl-10 m-3 ">
                  <Form onSubmit={sendDataUpdateAbout}>
...
export default UpdateDataAbout;
